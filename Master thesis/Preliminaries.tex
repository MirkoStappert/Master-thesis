...
\begin{lem}  \label{PropertiesTHH}
    ...
    3.) For any ordinary ring $R$, the natural map $\THH(R)\to \HH(R)$ induces an isomorphism on $\pi_0,\pi_1,\pi_2$ and a surjection on $\pi_3$. More generally\todo{Check whether this is true}, for a commutative, connective ring spectrum $S$ and an ordinary $\pi_0(S)$-algebra $R$, the same hold for the map $\THH(R/S)\to \HH(R/\pi_0(S))$. \\
    ...
    $R$ a connective, $p$-complete $S$-algebra spectrum, which is of finite type, then $\THH(R/S)$ is also $p$-complete.\\   
    5.) If $S$ is a connective $\Einf$-ring spectrum, $R$ a connective, $p$-complete $S$-algebra spectrum such that $R$ is of finite type over $S_p^{\wedge}$, then 
    \begin{equation*}
