...
%, which is not the case for (co)limits in the category of topological spaces. 
...
\end{equation*}
The existence of such a functor $N(\Z_{\geq 0})^{\op}\ra \Cat_\infty$ is not obvious. 
...
This construction refines into a fully faithful functor $H:\Ab\ra \Sp$ (\cite[Example~1.3.3.5]{lurie2017higher}). Therefore, we will often abuse notation and denote the Eilenberg-Maclane spectrum of $R$ simply by $R$ itself unless we want to stress that we are talking about a discrete ring, in which case we will write $HR$. Spaces also give rise to spectra under the \textbf{suspension spectrum} functor $\Sigma^{\infty}:\Spc_*\ra \Sp$. 
...
Similarly to spaces, spectra also have \textbf{homotopy groups}, which are defined via the ordinary homotopy groups of their spaces but now we can also define negative homotopy groups. For $X\in \Sp$ and $i\in\Z$ define $\pi_i(X)=\pi_{i+k}(X_k)$ as long as $i+k\geq 0$. This is independet of the choice of $k$ because $X_k\simeq \Omega X_{k+1}$. They are all abelian because we can always write them as higher homotopy groups of some space.
...
An immediate observation is that $p$-completion commutes with arbitrary limits and finite colimits. The crucial Lemma about $p$-complete spectra and the main reason why we have introduced the concept, is the following.
\begin{lem} \label{modpreduction}
    ...
    If furthermore $X$ and $Y$ are bounded below, the map $f$ is already a $p$-adic equivalence provided that $X\otimes_\S H\F_p\to Y\otimes_\S H\F_p$ is an equivalence (i.e. we can check it on $\F_p$-homology).
\end{lem}
...
\begin{itemize}
    ...
    \item For every spectrum $X$, the spectrum $X/p$ is $p$-complete. This implies that $X/p\to X_p^{\wedge}/p$ is an equivalence, because $p$ completion commutes with finite colimits.
...
\begin{lem}  \label{PropertiesTHH}
    ...
    4.) If $S$ is a connective $\Einf$-ring spectrum and %\footnote{This means, that $R$ is a filtered colimit of perfect $A$-module spectra along increasingly connective maps.}
    $R$ a connective, $p$-complete $S$-algebra spectrum, which is of finite type over $S$, then $\THH(R/S)$ is also $p$-complete.\\   
    5.) If $S$ is a connective $\Einf$-ring spectrum, $T$ a connective $\Einf$-$S$-algebra such that $S\to T$ is a $p$-adic equivalence and $R$ a connective, $p$-complete $T$-algebra , which is of finite type over $T$, then 
    \begin{equation*}
        \THH(R/S;\Z_p)=\THH(R/T).
    \end{equation*}
\end{lem}
...
\begin{proof}
    ...
    5.) The map $S\to T$ induces a map $\THH(R/S)\to \THH(R/T)$ and want to show that it is a $p$-adic equivalence. This suffices to show the statement, since the target is already $p$-complete by part 4.). 
    By Lemma \ref{modpreduction} we thus need to see that the map on mod $p$-reductions $\THH(R/S)/p\to \THH(R/T)/p$ is an equivalence. For this we can for example use Proposition \ref{base change THH}:
    \begin{align*}
        \THH(R/S)/p&=\THH(R/S)\otimes_S(S/p)\stackrel{\ref{base change THH}}{=}\THH((R/p)/(S/p))\\
        \to \THH(R/T)/p&=\THH(R/T)\otimes_{T}(T/p) \stackrel{\ref{base change THH}}{=}\THH((R/p)/(T/p))
    \end{align*}
    And this map is an equivalence, because by Lemma \ref{modpreduction} $S/p\to T/p$ is an equivalence.
\end{proof}
...
\\
Let us also address the monoidality of $\THH$.
...
%The appendix provides a short recollection on the definition of K-theory.\\ 
...
\begin{defn}
    ...
    \begin{equation*}
        X_{hG}\coloneqq \colim(F:BG\to \Sp),\quad
        X^{hG}\coloneqq \lim(F:BG\to \Sp) 
    \end{equation*}
    There is also a third functor that we need, called the \textbf{Tate construction} $X^{tG}$, which comes equipped with a natural map from the fixed points $X^{hG}\xrightarrow{\mathrm{can}}X^{tG}$ . 
...
\begin{defn}
    ...
    \begin{equation*}
        \TC(X)\ra X^{hS^1}\xrightarrow{\prod_{p\in\pP}\left( (\varphi_p)^{hS^1}-\mathrm{can}^{hS^1} \right)}\prod_{p\in\pP}(X^{tC_p})^{hS^1} 
    \end{equation*}
    We implicitly consider the map $\mathrm{can}^{hS^1}$ as being precomposed with an identification of its domain $\mathrm{can}^{hS^1}:X^{hS^1}\simeq (X^{hC_p})^{hS^1}\ra(X^{tC_p})^{hS^1}$.
...
\begin{equation*}
    \TC(X)=\fib\left(  \TCm(X)\xrightarrow{\prod_{p\in\pP}\left( (\varphi_p)^{hS^1}-\textrm{can}^{hS^1} \right)}\prod_{p\in\pP} \TP(X)^{\wedge}_p\right).
\end{equation*}
...
%\todo{What is with $\mathrm{TC}(\mathbb{Z}[\pi_1(X)])$ }
