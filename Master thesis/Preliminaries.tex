...
\begin{itemize}
    ...
    \item In the case that all $R$, $X$ and $Y$ are all connective and one of the factors is of finite type and the other one is $p$-complete, then $X\otimes_R Y$ is also $p$-complete. %We then do not need to complete on the outside again.
\end{itemize}
...
\end{equation*}
\\
Let us record a few basic facts, that we will need, in the following Lemma.\todo{Prove Lemma Part 4}
