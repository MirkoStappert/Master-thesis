...
\end{equation*}
\\
Let us record a few basic facts, that we will need, in the following Lemma.\todo{Prove Lemma and Proposition}
\begin{lem}
    1.) $\THH(\S)=\S$ \\
    2.) $\THH(R)$ is connective, if $R$ is connective. \\
    %3.) If $R$ is a $p$-complete spectrum and of finite type\footnote{This means, that $R$ is a filtered colimit of perfect $A$-module spectra along increasingly connective maps.} over $S$, then $\THH(R/S)$ is also $p$-complete.\\
    3.) Let $A\to B$ be a map of connective commutative ring spectra and $R$ an $\Eone$-$B$-algebra. Assume that $A\to B$ is $i$-connective, then $\THH(R/A)\to \THH(R/B)$ is $i+1$-connective. \todo{Check}\\
...
\begin{proof}
    ...
    %3.) By assumption $R$ is a filtered colimit of perfect $A$-module spectra $X_i$ along maps $X_i\ra X_{i+1}$ that are $i$-connective. 
...
\\
We also need to adress the monoidality properties of $\THH$, which is slightly more complicated.
\begin{prop}
